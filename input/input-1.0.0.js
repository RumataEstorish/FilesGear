/*global $, Utils, GearModel, pickText, KeyboardModes, tau, input*/
...
/**
 * Input wrapper. Shows input field depending on Gear model and prefered settings
 * 
 * @param model -
...
function Input(model) {
	...
	try {
		var self = this, height = window.innerHeight;
		...
		this.mode = KeyboardModes.NORMAL;

		this.__defineGetter__("windowStartHeight", function() {
			return height;
		});
		...
		$(window).on("resize", function() {
			var placeholder = "", page = Utils.getActivePage(), inputField = null;

			try {
				switch (page) {
				case 'inputCirclePage':
					try {
						inputField = self.getInputField();
